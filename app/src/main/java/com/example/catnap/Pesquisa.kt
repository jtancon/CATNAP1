...
class Pesquisa : AppCompatActivity() {
    ...
    private fun loadProductsFromDB(): MutableList<Product> {
        ...
        if (cursor.moveToFirst()) {
            do {
                val name =
                    cursor.getString(cursor.getColumnIndexOrThrow(ProductDatabase.COLUMN_PRODUCT_NAME))
                val descricao =
                    cursor.getString(cursor.getColumnIndexOrThrow(ProductDatabase.COLUMN_PRODUCT_DESC))
                val preco =
                    cursor.getDouble(cursor.getColumnIndexOrThrow(ProductDatabase.COLUMN_PRODUCT_PRICE))
                        .toString()
